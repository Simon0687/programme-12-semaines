...
/* Roue de charge (#46) — la partie qui touche au DOM.
   ...
   charge** suit le doigt au pixel mais n'a pas d'inertie, puisque c'est nous
...
/* Posé en permanence sur les champs de charge, pas au moment de l'ouverture —
...
export const LOAD_FIELD_STYLE = { touchAction: "none", WebkitTouchCallout: "none" };

export function useLoadPicker({ incr, onCommit }) {
  const [picker, setPicker] = useState(null); // { anchor, steps, x, y } | null
  ...
  const handlers = (id, field, current, plan) => {
    ...
    return {
      onPointerDown: (e) => {
        ...
        g.current = { id, field, anchor, steps: 0, x: e.clientX, y: e.clientY, lastY: e.clientY, mode: "pending" };
