...
Cloudflare Pages construit une preview de la branche : URL `staging.programme-12-semaines.pages.dev` (accessible en 2–3 min).
...
**Automatiquement :**
- ✅ Build et tests
- 🚀 Déploiement Cloudflare Pages en production (URL fixe)
...
Cloudflare Pages déploie sur `staging.programme-12-semaines.pages.dev`.
...
Automatiquement :
- 🚀 Déploiement sur `programme-12-semaines.pages.dev` (URL fixe)
...
| **staging** | Pré-production, tests | Preview Cloudflare Pages (alias de branche) | URL preview (partage facile) |
| **main** | Production | Cloudflare Pages prod (fixe) | Endpoint public |
...
Aucun. Cloudflare Pages est branché sur le dépôt GitHub en OAuth : c'est Cloudflare
...
Cloudflare, *Settings → Builds & deployments*. Seule la version de Node est dans le
...
- **Cloudflare Pages** : Logs de build, historique des déploiements, rollback en un clic
...
| Jeu 17h | Simon teste sur la preview `staging` | URL Cloudflare Pages |
