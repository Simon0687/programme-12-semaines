...
import { V, SLOTS, SESSIONS } from "../src/program.js";
...
const si = (sid) => SESSIONS.findIndex((s) => s.id === sid);
...
describe("calibration", () => {
  ...
  test("all sets at the top of the range at >= 3 RIR: +5%, snapped to incr", () => {
    const p = planned(wk1(set(72.5, 8, 3), set(72.5, 8, 3), set(72.5, 8, 3)), "dc", 2, si("hautA"));
    assert.equal(p.load, 75); // roundTo(72.5 * 1.05, 2.5)
    assert.equal(p.load % V.dc.incr, 0);
    assert.equal(p.why, "calibration : +5 %");
  });

  test("one set below the bottom of the range: -5%", () => {
    const p = planned(wk1(set(72.5, 3, 2), set(72.5, 6, 2), set(72.5, 6, 2)), "dc", 2, si("hautA"));
    assert.equal(p.load, 70); // roundTo(72.5 * 0.95, 2.5)
    assert.equal(p.why, "calibration : −5 %");
  });

  test("in range but not maxed: load held", () => {
    const p = planned(wk1(set(72.5, 6, 2), set(72.5, 6, 2), set(72.5, 6, 2)), "dc", 2, si("hautA"));
...
describe("progression", () => {
  test("all sets maxed at <= 1 RIR: + the variant increment, not %-rounded", () => {
    ...
    assert.equal(p.load, 75 + V.dc.incr); // 77.5, exact
  ...
  test("lower-body increment (squat, +5 kg)", () => {
    ...
    assert.equal(p.load, 105 + V.squat.incr); // 110
  });
});
...
describe("week 7 deload", () => {
  ...
  test("week 8 resumes from the pre-deload session, not the deload load", () => {
    ...
    assert.equal(p.load, 100 + V.dc.incr); // 102.5, from week 6
    assert.notEqual(p.load, 85 + V.dc.incr); // not from the week-7 deload load
    assert.match(p.why, /^\+2,5 kg/);
  });
});
...
describe("new block-2 variant in week 7", () => {
  test("week 7: no prior history, shows the ramp-up branch", () => {
    const p = planned(S(), "tristretch", 7, si("hautA")); // b2 = skull, no start
    assert.equal(p.load, null);
    assert.equal(p.text, "Paliers");
  });
...
describe("same variant twice in one week", () => {
  test("the second session accounts for the first (e.si < si)", () => {
    ...
    const first = planned(st, "latraise", 5, si("hautA")); // si 0: does not see itself
    const second = planned(st, "latraise", 5, si("hautC")); // si 3: sees Haut A
    assert.equal(first.text, "Paliers"); // no history yet
    assert.equal(second.load, 14 + V.lat_db.incr); // 16, progressed off Haut A
  });
});
...
describe("pull-ups (unit bw)", () => {
  test("no added load reads as bodyweight", () => {
    const p = planned(S(), "pull", 1, si("hautB"));
  ...
  test("maxed at <= 1 RIR at bodyweight: suggestion becomes bodyweight + increment", () => {
    ...
    assert.equal(p.load, 0 + V.pullup.incr); // 2.5
...
describe("history / lastEntry", () => {
  ...
  test("lastEntry ignores the current session and later ones", () => {
    ...
    assert.equal(lastEntry(st, "lat_db", 4, si("hautA")).week, 2);
  });
});
